...
import { DashboardShell, type Tab } from "./pages/Dashboard/DashboardShell.tsx";
import { TodayGlowDashboard } from "./pages/Dashboard/TodayGlowDashboard.tsx";
...
import { AddressWelcome } from "./pages/Landing/AddressWelcome.tsx";
import { WhyItMattersPage } from "./pages/WhyItMatters/WhyItMattersPage.tsx";
...
export default function App() {
  ...
  const days = useMemo(
    () => splitByLocalDay(estimates, timezone),
    [estimates, timezone],
  );
  ...
  const completeWithSuggestion = useCallback(
    async (s: AddressSuggestion) => {
      ...
      if (phase === "address" && !isWelcomeEmailValid(welcomeEmail)) {
        setError(
          "Enter a valid email, then choose a place or tap Get My Solar Forecast.",
        );
        return;
      }
      try {
        const { latitude, longitude, label } =
          await resolveAddressSuggestion(s);
        ...
        setError(
          e instanceof Error ? e.message : "Could not resolve this place.",
        );
      }
    },
    ...
    navigator.geolocation.getCurrentPosition(
      async (pos) => {
        ...
        if (phase === "address") {
          setPlaceQuery("");
          setGeoStatus(
            "Location saved. Enter your email, then tap Get My Solar Forecast.",
          );
          return;
        }
        const ok = await runForecast(la, lo);
  ...
  const main = whyMatters ? (
    <WhyItMattersPage
      onViewDashboard={() => goTab("flow")}
      onGetSmartAlerts={() => goTab("sms")}
    />
